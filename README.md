...
*   **🧹 智能清洗与过滤**:
    ...
    *   **🤖 AI/流媒体优化**: 自动剔除 **中国大陆(CN)、俄罗斯(RU)、伊朗(IR)** 等无法访问 OpenAI/Claude/Google 的地区节点。
    *   **⚡️ 全量连通性检测**: 每次更新时，不仅检测新节点，还会**重新检测 Gist 中所有存量节点**。
    *   **🔒 真实性验证**: 
        *   **Trojan 真连接**: 对 Trojan 节点发起真实的 Google 连接测试 (`CONNECT www.google.com:80`)，确保能翻墙。
        *   **HTTP 活跃探测**: 对其他节点进行 HTTP 活跃探测，拒绝“僵尸”节点。
    *   **♻️ 自动淘汰**: 流量耗尽或失效的旧节点会被自动剔除，确保池内节点 100% 可用。
*   **📂 智能排序**: 自动按协议优先级 (Trojan > VMess/SS > VLESS) 和名称排序，让列表井井有条。
*   **📦 容量管理**: 自动维护 Gist 列表，保留最新的 500 个高质量节点。
...
4.  **清洗**:
    *   解码 Base64 订阅内容。
    *   **过滤**: 深度扫描节点名称，剔除 "HK", "CN", "RU", "免费", "套餐到期" 等无效或受限节点。
    *   **全量测速**: 使用多线程并行技术，对新获取的节点 + Gist 中已有的所有节点进行检测。
    *   **深度验证**: 
        *   **TCP 握手**: 基础连通性。
        *   **SSL 握手**: 针对 TLS 节点验证证书握手。
        *   **真连接测试**: Trojan 节点尝试连接 Google；其他节点进行 HTTP 活跃探测。
    *   **优胜劣汰**: 任何无法连接的节点（包括刚获取的坏节点或已失效的老节点）都会被直接丢弃。
    *   **优胜劣汰**: 任何无法连接的节点（包括刚获取的坏节点或已失效的老节点）都会被直接丢弃。
5.  **排序**: 按协议优先级 (Trojan > VMess > VLESS) 和名称进行排序。
6.  **发布**: 将清洗后的优质节点列表（最多 500 个）推送到云端。
